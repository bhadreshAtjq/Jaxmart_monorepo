...
import { Card, Button, Container, Input, Textarea } from '@/components/ui';
...
export default function NewListingPage() {
  ...
  const handleCreate = async () => {
    setLoading(true);
    try {
      // 1. Gather all specifications (Category Attribute inputs + Custom Spec rows)
      ...
      // 2. Prepare payload matching listingController structure
      const payload: any = {
        listingType,
        title,
        description,
      ...
      if (listingType === 'PRODUCT') {
        ...
        payload.minOrderQty = minOrderQty;
        payload.pricePerUnit = pricePerUnit;
        payload.priceType = priceType;
        payload.priceRangeMin = priceRangeMin;
        payload.priceRangeMax = priceRangeMax;
        payload.bulkPriceSlabs = bulkPriceSlabs;
        payload.stockAvailable = true;
        payload.leadTimeDays = leadTimeDays;
        payload.hsnCode = hsnCode;
        payload.gstRate = gstRate;
        payload.specifications = finalSpecifications;
        payload.supplyAbility = supplyAbility;
        payload.deliveryTime = deliveryTime;
        payload.packagingDetails = packagingDetails;
        payload.paymentTerms = paymentTerms;
        payload.fobPort = fobPort;
        payload.sampleAvailable = sampleAvailable;
        payload.samplePrice = samplePrice;
        ...
        payload.typicalDuration = typicalDuration;
        payload.teamSize = teamSize;
        payload.avgResponseHrs = avgResponseHrs;
        payload.languages = languages.split(',').map(l => l.trim()).filter(Boolean);
        payload.skillsTags = skillsTagsInput.split(',').map(s => s.trim()).filter(Boolean);
        payload.basePrice = packages[0]?.price || 0; // Use Basic tier price as base
        payload.priceUnit = 'PROJECT';
        payload.currency = 'INR';
        payload.packages = packages;
      }

      await listingApi.create(payload);
      toast.success('Inventory Cataloged & Indexed Successfully');
      router.push('/seller/listings');
    } catch (err: any) {
      toast.error(err?.response?.data?.message || 'Provisioning sequence failed');
    } finally {
      setLoading(false);
    }
  };
                      ...
                      {(['PRODUCT', 'SERVICE'] as const).map(type => (
                        <button
                          ...
                          className={clsx(
                            ...
                            listingType === type ? "border-jax-accent bg-jax-accent/5" : "border-gray-100 bg-white hover:border-gray-200"
                          )}
                        >
                          <div className={clsx(
                            "h-16 w-16 rounded-2xl mx-auto mb-6 flex items-center justify-center transition-all",
                            listingType === type ? "bg-jax-accent text-white" : "bg-gray-50 text-gray-300 group-hover:bg-gray-100"
                          )}>
