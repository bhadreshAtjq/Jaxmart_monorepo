...
import { useAdminStats, useAdminUsers, useAdminKycQueue, useAdminListingsQueue, useAdminEvents, revalidate } from '@/lib/hooks';
               ...
               {tab === 'listings' && (
                  <div className="space-y-6">
                     <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-4 border-b border-gray-100">
                        <div>
                           <h2 className="text-xl font-heading font-black text-jax-dark tracking-tight">Inventory Review Stream</h2>
                           <p className="text-xs text-gray-400 mt-1">Audit and approve catalog products submitted by field captains & sellers</p>
                        </div>
                     </div>
                     {listingsLoading ? (
                        <div className="space-y-4"><Skeleton className="h-32" /><Skeleton className="h-32" /></div>
                     ) : (listingsQueue?.queue || []).length === 0 ? (
